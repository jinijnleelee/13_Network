...
public class TCPServer {
	...
	//TCP Socket 프로그래밍
	//TCP : 데이터 전달의 신뢰성을 최대한 보장하기 위한 방식(연결 지향형 등신)
	//	 순차적으로 데이터를 전달하고 확인 및 오류 시 재전송
	
	// Socket : 프로세스의 통신에 사용되는 양 끝단.
	//		서버와 클라이언트가 통신을 하귀휘한 매개체
	
	// ServerSocket : 서버용 소켓
			//- port와 연결되어 외부 요청을 기다리는 역할
			//-> 클라이언트 요청이 올경우이를 수락(accept)하고
	...
	public void serverStart() {
		...
		BufferedReader br = null; // 익명용 보조 스트림 변수
		
		OutputStream os = null;	// 서버 -> 클라이언트 출력용 스트림 변수
		PrintWriter pw = null;
		//BufferedWriter br = null;
		//PrintWriter에는 println, print, printf 등 다양한 출력함수를 제공함으로써 
		//파일 출력을 편하게 해준다.
		...
		try {
			...
			//5.연결된 클라이언트와 입출력 스트림 생성
			 ...
			 os = clientSocket.getOutputStream(); // clientSocket에서 제공하는 스티림
			...
			//7.스트림을 통해 읽고 쓰기
			 ...
			 // close()도 밀어내는게 가능한데, 굳이 flush를 사용하는 까닭
